...
BUILD_DIR="/tmp/kindlewall_verify_t90"
mkdir -p "$BUILD_DIR"
MODULE_CACHE_DIR="$(mktemp -d /tmp/kindlewall_module_cache_t90.XXXXXX)"
trap 'rm -rf "$MODULE_CACHE_DIR"' EXIT

OUTPUT_BIN="$BUILD_DIR/verify_t90"
