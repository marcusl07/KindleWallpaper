...
require_pattern "$APP_STATE_FILE" 'func[[:space:]]+reapplyStoredWallpaperIfAvailable\(\)[[:space:]]*->[[:space:]]*Bool' "app state wake reapply API"
...
require_pattern "$WALLPAPER_SETTER_FILE" 'func[[:space:]]+tryReapplyStoredWallpapers' "stored wallpaper reapply entrypoint"
