...
require_pattern "$SETTINGS_FILE" "showBooksAction:[[:space:]]+\\(\\(\\)[[:space:]]*->[[:space:]]*Void\\)\\?" "optional show-books action injection"
require_pattern "$SETTINGS_FILE" "closeBooksAction:[[:space:]]+\\(\\(\\)[[:space:]]*->[[:space:]]*Void\\)\\?" "optional close-books action injection"
require_pattern "$SETTINGS_FILE" "if[[:space:]]+let[[:space:]]+showBooksAction" "Show Books action preference path"
